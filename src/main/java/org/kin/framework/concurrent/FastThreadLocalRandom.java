    ...
    static {
        if (initialSeedUniquifier == 0) {
            boolean secureRandom = AccessController.doPrivileged(new PrivilegedAction<Boolean>() {
                @Override
                public Boolean run() {
                    return SysUtils.getBoolSysProperty("java.util.secureRandomSeed", false);
                }
            });
            ...
            if (secureRandom) {
                ...
                seedGeneratorThread.setUncaughtExceptionHandler(new UncaughtExceptionHandler() {
                    ...
                    public void uncaughtException(Thread t, Throwable e) {
                        log.debug("An exception has been raised by {}", t.getName(), e);
                    }
                });
        ...
        synchronized (FastThreadLocalRandom.class) {
            ...
            for (; ; ) {
                ...
                try {
                    ...
                    interrupted = true;
                    log.warn("Failed to generate a seed from SecureRandom due to an InterruptedException.");
                    break;
                }
