...
/**
 * 1. wheel timer slot: 链表结构, O(1), 相比, {@link java.util.concurrent.DelayQueue}和{@link java.util.Timer}, O(log n)
 *      插入和删除调度任务的性能更高.
 * 2. {@link #newTimeout(TimerTask, long, TimeUnit)}并不会把task插入wheel timer slot, 而缓存在一个queue里面, 等待tick才处理
 * 3. 每个wheel timer带一个worker, 模拟时钟tick, 其核心逻辑如下:
 *      1. {@link Worker#processCancelledTasks()}, cancel task并不会马上从wheel timer slot移除, 而是缓存在一个queue里面, 每次tick才将cancelled task真正从wheel timer slot上移除
...
public class HashedWheelTimer implements Timer {
    ...
    public HashedWheelTimer(
        ...
        Preconditions.checkNotNull(threadFactory, "threadFactory must be not null");
        Preconditions.checkNotNull(unit, "unit must be not null");
        Preconditions.checkArgument(tickDuration > 0, "tickDuration must be not greater than 0");
        Preconditions.checkNotNull(ticksPerWheel > 0, "ticksPerWheel must be not greater than 0");
    ...
    public Timeout newTimeout(TimerTask task, long delay, TimeUnit unit) {
        Preconditions.checkNotNull(task, "task must be not null");
        Preconditions.checkNotNull(unit, "unit must be not null");
